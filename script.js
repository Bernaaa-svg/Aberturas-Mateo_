...
// La función se llama al hacer click en el botón
function calcularPrecio() {
    ...
    const precioDolarKg = parseFloat(document.getElementById('precioDolarKg').value);

    // 2. Validar que los valores sean números positivos
    if (isNaN(pesoKg) || isNaN(alturaMm) || isNaN(precioDolarKg) || pesoKg <= 0 || alturaMm <= 0 || precioDolarKg <= 0) {
        document.getElementById('valor-dolar').textContent = "Error: Ingresa valores válidos";
        return;
    }

    // 3. Variables de cálculo
    // Nota: Aunque el peso ya está en kg, en un cálculo real la altura (mm) podría usarse
    // para estimar la cantidad de material requerido o para cálculos de ingeniería.
    // Para simplificar, asumiremos que el costo se basa directamente en el peso (kg)
    // y la altura se usa como una variable de entrada adicional.

    // 4. Realizar el cálculo del costo total en USD
    // Costo Total = Peso (kg) * Precio por kg (USD/kg)
    const costoTotalDolar = pesoKg * precioDolarKg;
    
    // 5. Formatear el resultado a dos decimales
    const costoFormateado = costoTotalDolar.toFixed(2);

    // 6. Mostrar el resultado en el HTML
    document.getElementById('valor-dolar').textContent = costoFormateado;
}